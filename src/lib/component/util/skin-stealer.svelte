<script lang="ts">
    ...
    onMount(async () => {
        ...
        if (ign) {
            searchValue = ign
            link = "https://mcdevtools.tr/skin-stealer#ign=" + ign
    ...
    const handleInput = () => {
        link = "https://mcdevtools.tr/cape-stealer#ign=" + searchValue
    };

    let link = "https://mcdevtools.tr/skin-stealer#ign="

    function downloadSkin() {
        toast.push('Başarılı bir şekilde indirildi!', {
            theme: {
...
<div class="flex gap-3">
    <input class="search w-[26rem]" maxlength="16" bind:value={searchValue} on:input={handleInput} on:keydown={disallowSpaces} type="text" placeholder="Kullanıcı adı giriniz..." on:keypress={handleKeyPress} on:blur={handleInput}>
    <button class="button text-md py-0" on:click={() => updateSkin(searchValue)}>Arat</button>
</div>
...
{:else if noAccountFound}
    <p class="mt-10 text-[#F55050] text-2xl">Bu isimde bir kullanıcı bulunamadı.</p>
{:else}
    <canvas bind:this={skinContainer} class="mt-10 "></canvas>
    <div class="flex gap-6">
        <a href={currentSkin} download="" aria-label='Download Skin'><button class="button" on:click={downloadSkin}>Cildi İndir</button></a>
        <a href="https://www.minecraft.net/en-us/msaprofile/mygames/editskin" aria-label='Apply Skin' target="_blank"><button class="button">Cildi Uygula</button></a> <!-- Mojang broke passing the image through the URL. NameMC removed it & other sites don't work either. So just sending a template link -->
    </div>
{/if}

{#if !loading && currentSearch}
    <div class="flex flex-col mt-8 w-fit">
        <h3 class="font-medium text-white text-20px text-center">Paylaşılabilir Bağlantı</h3>
        <div class="flex gap-3 mt-2 w-fit">
            ...
            <button on:click={copyLink} class="w-fit text-sm px-2 py-1.5 button h-fit inline-block">Kopyala</button>
        </div>
    </div>
{/if}
