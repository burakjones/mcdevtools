...
<div class="place-items-center text-center items-start grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 gap-10">
    <div class="flex flex-col">
        <h3 class="font-medium text-white text-[20px] text-left">Tür</h3>
    ...
    <div class="flex flex-col">
        <h3 class="font-medium text-white text-[20px] text-left">Sürüm</h3>
    ...
    <a href={selectedDownloadUrl} aria-label='Download Jar' class="self-end">
        <button class="button h-fit" on:click={downloadSuccess}>İndir</button>
    </a>
</div>
...
{:then versions}
    <table class="w-[90%] lg:w-[60%] text-white mt-12">
        <tr class="border-b-[1.5px] border-b-[#232324] text-[#9d9d9e]">
            <th class="rounded-tl-lg rounded-bl-lg p-2 font-medium text-left">{selectedType === "velocity" ? 'Version' : 'Minecraft Sürümü'}</th>
            <th class="font-medium text-left">Yayınlanma Tarihi</th>
        </tr>
        {#each versions as version}
