<script lang="ts">
    ...
    const handleInput = () => {
        link = "https://mcdevtools.tr/server-info#ip=" + searchValue
    ...
    let link = "https://mcdevtools.tr/server-info#ip="
    function copyLink() {
    ...
    onMount(() => {
        ...
        if (server) {
            searchValue = server
            link = "https://mcdevtools.tr/server-info#ip=" + server
            fetchServerStatus()
        }
    })
</script>

<input class="search w-[26rem] mb-12" on:keydown={disallowSpaces} bind:value={searchValue} on:input={handleInput} type="text" placeholder="Bir sunucu adresi girin..." on:blur={handleInput}>
...
{:else}
    <div class="self-center relative w-[650px] transition-transform transform scale-[55%] sm:scale-[80%] md:scale-[100%] lg:scale-[100%] xl:scale-100">
        ...
        <div class="mr-auto text-start motd-content overflow-y-hidden">
            <p class="motd-text text-start text-[#AA0000]"><br>Sunucuya bağlanılamıyor...</p>
        </div>
