<script lang="ts">
    ...
    function calculateResult() {
        ...
        let windows = java
        if (autoRestart) {
            windows = ":start<br>" + windows + "<br><br>Sunucu yeniden başlatılıyor...<br>goto :start";
        }
        tabData[0].content = windows

        let linuxMac
        if (autoRestart) {
            linuxMac = "#!/bin/bash<br><br>while [ true ]; do<br>&emsp;&emsp;&emsp;&emsp;" + java + "<br><br>&emsp;&emsp;&emsp;&emsp;echo Sunucu yeniden başlatılıyor...<br>done"
        } else {
...
<div class="place-items-center text-center items-start grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-10 mt-3">
    <div class="flex flex-col">
        <h3 class="font-medium text-white text-[20px] text-left">Sunucu Jar İsmi</h3>
    ...
    <div class="flex flex-col">
        <div class="flex flex-row gap-4">
            ...
            <button on:click={CalculateRam} class="button text-sm w-fit px-2 py-1">Hesapla</button>
        </div>
    ...
    <div class="flex flex-col w-[160px]">
        ...
        <select bind:value={flags} on:change={calculateResult} id="flags" class=" py-2.5 px-0 w-full text-sm bg-transparent border-0 border-b-2 border-gray-200 text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-gray-200 peer">
            <option value="none" class="bg-[#3C414B] text-gray-400 hover:text-white hover:bg-[#626875]" >Yok</option>
            <option value="aikar" class="bg-[#3C414B] text-gray-400 hover:text-white hover:bg-[#626875]">Aikar's Flags</option>
            <option value="aikarex" class="bg-[#3C414B] text-gray-400 hover:text-white hover:bg-[#626875]">Aikar's Flags (Gelişmiş 12gb+)</option>
        ...
        <label class="relative inline-flex items-center mb-4 cursor-pointer">
            ...
            <p class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">
                <a href="#"
                   ...
                   title="This starts an inbuilt GUI control panel as well as the console.">Ek GUI</a>
            </p>
        </label>
        <label class="relative inline-flex items-center mb-4 cursor-pointer">
            ...
            <p class="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">
                <a href="#"
                   ...
                   class="text-white transition duration-150 ease-in-out hover:text-primary-600 focus:text-primary-600 active:text-primary-700 dark:text-primary-400 dark:hover:text-primary-500 dark:focus:text-primary-500 dark:active:text-primary-600"
                   data-te-toggle="tooltip"
                   title="This restarts the server after it has crashed or stopped.">Otomatik Yeniden Başlatma</a>
            </p>
        </label>
    </div>
</div>
...
<div class="flex flex-col w-[90%] lg:w-[60%]">
    <h3 class="font-medium text-white text-[20px] mt-12 text-center mb-5">Sonuç</h3>
    <div class="tab-system w-[100%] flex flex-col">
        ...
        <div class="flex justify-center gap-3 mt-3">
            <button class="w-fit text-sm px-2 py-1.5 button" on:click={copyValue}>Kopyala</button>
            <button class="w-fit text-sm px-2 py-1.5 button" on:click={downloadStartBat}>İndir</button>
        </div>
    </div>
</div>
