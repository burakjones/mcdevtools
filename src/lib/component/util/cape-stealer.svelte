<script lang="ts">
    ...
    onMount(async () => {
        ...
        if (ign) {
            updateSkin(ign)
            link = "https://mcdevtools.tr/cape-stealer#ign=" + ign
        }
    })
    ...
    let link = "https://mcdevtools.tr/cape-stealer#ign="
 ...
 <div class="flex gap-3">
    <input class="search w-[26rem]" maxlength="16" bind:value={searchValue} on:input={handleInput} on:keydown={disallowSpaces} type="text" placeholder="Lütfen bir kullanıcı adı girin..." on:keypress={handleKeyPress} on:blur={handleInput}>
 ...
 {:else if noCapes}
    <p class="mt-10 text-[#F55050] text-2xl">Bu hesaba ait bir pelerin bulunamadı.</p>
 {:else}
        ...
        {#each data as cape (cape.hash)}
            {#if cape.exists && cape.frontImageUrl}
                <div class="flex flex-col items-center justify-center">
                    ...
                    <a href="" aria-label='Download Skin' class="mt-4" on:click={() => downloadSkin(cape.imageUrl)}><button class="button">İndir</button></a>
                </div>
            {/if}
        {/each}
    </div>
