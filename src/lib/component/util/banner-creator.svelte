<script lang="ts">
    ...
    function clearLayer(){
        setColor("")
        setPattern("")
        toast.push('Katman başarıyla temizlendi!', {
            theme: {
    ...
    function clearBanner(){
        ...
        updateOutput()
        toast.push('Afiş başarıyla temizlendi!', {
            theme: {
                '--toastColor': 'mintcream',
                '--toastBackground': 'rgba(72,187,120,0.9)',
                '--toastBarBackground': '#2F855A'
    ...
    function undo() {
        if (history.length > 1) {
            ...
            toast.push('Başarıyla geri sarıldı!', {
                theme: {
                    '--toastColor': 'mintcream',
                    '--toastBackground': 'rgba(72,187,120,0.9)',
                    '--toastBarBackground': '#2F855A'
    ...
    function updateOutput() {
        ...
        link = "https://mcdevtools.tr/banner-creator#base=" + tabData[0].color
    ...
    <div class="grid grid-cols-1 xl:flex xl:flex-wrap w-full gap-12">
        ...
        <div class="w-full min-w-[10px] flex-1">
            <div class="flex flex-col">
                <h3 class="font-medium text-white text-20px text-left">Afiş Verme Komutu</h3>
                <div class="flex gap-3 mt-2">
                    ...
                    <button on:click={() => copyValue(bannerGive)} class="w-fit text-sm px-2 py-1.5 button h-fit inline-block">Kopyala</button>
                </div>
            </div>
            <div class="flex flex-col mt-6">
                <h3 class="font-medium text-white text-20px text-left">Kalkan Verme Komutu</h3>
                <div class="flex gap-3 mt-2">
                    ...
                    <button on:click={() => copyValue(shieldGive)} class="w-fit text-sm px-2 py-1.5 button h-fit inline-block">Kopyala</button>
                </div>
            </div>
            <div class="flex flex-col mt-6">
                <h3 class="font-medium text-white text-20px text-left">Spigot API Kodu</h3>
                <div class="flex gap-3 mt-2">
                    ...
                    <button on:click={() => copyValue(spigot)} class="w-fit text-sm px-2 py-1.5 button h-fit inline-block">Kopyala</button>
                </div>
            </div>
            <div class="flex flex-col mt-6">
                <h3 class="font-medium text-white text-20px text-left">Paylaşılabilir Bağlantı</h3>
                <div class="flex gap-3 mt-2">
                    ...
                    <button on:click={() => copyValue(link)} class="w-fit text-sm px-2 py-1.5 button h-fit inline-block">Kopyala</button>
                </div>
            </div>
        </div>
    </div>
    <div class="flex items-center justify-center mt-8">
        <button on:click={clearLayer} class="text-sm px-2 py-1.5 button mr-2">Katmanı Temizle</button>
        <button on:click={clearBanner} class="text-sm px-2 py-1.5 button mr-2">Afişi Temizler</button>
        <button on:click={undo} class="text-sm px-2 py-1.5 button">Geri Al</button>
    </div>
</main>
